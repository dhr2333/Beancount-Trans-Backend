...
10. 结构/明细分析：先用聚合查询（GROUP BY account 或 payee）定位重点，再可选第二条明细查询；若结果提示「已截断」，必须改用 GROUP BY 聚合重查，禁止对截断样本求和。
...
14. 同一问题最多调用 run_bql 3 次（系统硬限制）；若仍无满意结果，请根据已有查询结果作答，不要无限重试。
